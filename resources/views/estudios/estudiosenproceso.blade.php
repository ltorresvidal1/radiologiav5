...
@section('tituloformulario','Estudios en proceso')
...
<script>
  ...
  /********************************************************************************/
    ...
    url: '/assets/js/plugins/datatables/es-ES.json',
      ...
      autoWidth: false,
      ajax: {
            url: "{{route('datatable.estudiosenproceso')}}",
            data: function (d){
                d.sede=buscar_sedes,
                d.sala=buscar_salas,
...
function actulizar_filtros() {
    $('#tabletab3').DataTable().ajax.reload();
}
...
<script type="module">
/*
      Echo.channel('escuchandoestudiosenproceso').listen('estudioenprocesoEvent',(e) => {
         console.log("estudioenprocesoEvent"+e.message);
